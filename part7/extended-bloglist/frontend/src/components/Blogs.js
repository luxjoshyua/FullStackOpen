...
const Blogs = ({ user, logout, userId }) => {
	...
	if (user) {
		return (
			<div>
				{successMessage && <Notification className="success" message={successMessage} success={true} />}
				<div
					style={{
						display: 'flex',
				...
				{sortedBlogs.map((blog) => (
					<Blog key={blog.id} blog={blog} updateBlog={updateBlog} deleteBlog={deleteBlog} user={user} userId={userId} />
				))}
			</div>
		);
	} else {
