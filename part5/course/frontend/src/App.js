...
const App = () => {
  ...
  const addNote = (noteObject) => {
    ...
    noteService.create(noteObject).then((returnedNote) => {
      ...
      setSuccessMessage(
        `a new note titled "${returnedNote.content}" by user "${user.name}" successfully added`
      )
      setTimeout(() => {
  ...
  const toggleImportanceOf = (id) => {
    ...
    noteService
      ...
      .catch((error) => {
        setErrorMessage(
          `Error ${error} experienced, note '${note.content}' was already removed from server`
        )
        setTimeout(() => {
          setErrorMessage(null)
        }, 5000)
  ...
  const noteForm = () => (
    <Togglable buttonLabel="new note" ref={noteFormRef} id="new-note-btn">
